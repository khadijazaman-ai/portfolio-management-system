...
// Register a new user & seed defaults
exports.register = async (req, res) => {
  try {
    ...
    await new Notification({
      user: user._id,
      message: 'Welcome to your Portfolio CMS Dashboard! Please complete your profile.'
    }).save();
...
exports.login = async (req, res) => {
  try {
    ...
    // Create incomplete profile notification if completeness is low
    const fieldsToCheck = [
    ...
    const completeness = Math.round((filledCount / fieldsToCheck.length) * 100);
    if (completeness < 70) {
      await new Notification({
        user: user._id,
        message: `Your profile completeness is at ${completeness}%. Please fill in the remaining details.`
      }).save();
    }
